...
export default function page({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("Error while saving the invoice:", error);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (confirmed) {
      try {
        ...
        console.error("Error while deleting the invoice.", error);
      }
    }
  };
  ...
  if (!invoice || !totals)
    return (
      <div className="flex justify-center items-center h-screen w-full ">
        <span className="font-bold">Invoice not found</span>
      </div>
    );
          ...
          <div className="flex  w-full md:w-1/3 flex-col">
            <div className="mb-4 bg-base-200 rounded-xl p-5">
              <div className="flex justify-between items-center mb-4 ">
                <div className="badge badge-accent">Invoice Summary</div>
              ...
              <div className="flex justify-between">
                <span>
                  VAT ({invoice?.vatActive ? `${invoice?.vatRate}` : "0"} %)
                </span>
